    ...
    table: {
        __generateDTRow(data){ 
            return [
                ...
                data.name,     
                data.type == 1 ? `<span class="badge badge-pill badge-geekblue">Nguyên liệu thô</span>`  : `<span class="badge badge-pill badge-green">Đã sơ chế</span>`,     
        ...
        init(){
            var row_table = [ 
                                ...
                                { title: 'Tên nguyên liệu', name: 'name', orderable: true, },  
                                { title: 'Phân loại', name: 'name', orderable: true, width: '40%', },  
    ...
    FullTab: {  
        Create: { 
            ...
            getVal(resource){ 
                ...
                var data_type           = $(`${resource}`).find('.data-type').val().replace(noTag, "");  
...
(() => {
    ...
    View.FullTab.onPush("Delete", "#popup-delete", () => {
        ...
        View.FullTab.default();
        IndexView.helper.showToastProcessing('Process', 'Đang xử lí');
        Api.Ingredient.Delete($("#popup-delete").find('.data-id').val())
            .done(res => { 
                IndexView.helper.showToastSuccess('Success', 'Thành công');
                getData();
            })
