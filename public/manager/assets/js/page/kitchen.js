    ...
    table: {
        __generateDTRow(data){ 
            return [
                ...
                data.name,     
                data.coefficient,  
                data.valid == 0 ? `<span class="badge badge-pill badge-orange">Đang sử dụng</span>`  : `<span class="badge badge-pill badge-green">Còn trống</span>`,     
        ...
        init(){
            var row_table = [ 
                                ...
                                { title: 'Tên bếp', name: 'name', orderable: true, },  
                                { title: 'Hệ số', name: 'name', orderable: true, },  
                                { title: 'Khả dụng', name: 'name', orderable: true, width: '40%', },  
    ...
    FullTab: {  
        Create: { 
            ...
            getVal(resource){ 
                ...
                if (data_coefficient == '') { required_data.push('Nhập hệ số.'); onPushData = false }  
        ...
        Update: { 
            setVal(resource, data){
                ...
                $(`${resource}`).find('.data-coefficient').val(data.coefficient);
            },
            getVal(resource){ 
                var fd = new FormData();
                var required_data = [];
...
(() => {
    ...
    View.FullTab.onPush("Delete", "#popup-delete", () => {
        ...
        View.FullTab.default();
        IndexView.helper.showToastProcessing('Process', 'Đang xử lí');
        Api.Kitchen.Delete($("#popup-delete").find('.data-id').val())
            .done(res => { 
                IndexView.helper.showToastSuccess('Success', 'Thành công');
                getData();
            })
